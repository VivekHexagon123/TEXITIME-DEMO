...
WHERE id = ?`;
    db.query(query ,  [first_name, last_name,email, mobile ,avatar, userId]  , (err , result ) =>{
        if (err) {
...
const addnewprovider = ( userData,callback) =>{
    const { first_name, last_name, email, mobile,hashedPassword, avatar } = userData
    const query = `INSERT INTO providers (first_name, last_name, email, mobile, password, avatar) VALUES (?, ?, ?, ?, ?, ?)`
    db.query(query ,  [first_name, last_name, email, mobile, hashedPassword, avatar] , (err,result) =>{
        if (err) {
...
module.exports = {listproviders ,deleteprovider ,editgetproviders ,editprovider ,addnewprovider , getProviderStatus, updateProviderStatus , cheackemailexist , cheackemailexistupdate }
