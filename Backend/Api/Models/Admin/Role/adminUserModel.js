...
const listusers = (callback) => {
  const query = `SELECT id, first_name, last_name, email, mobile, wallet_balance 
  FROM users 
  ORDER BY id DESC`;
  // "SELECT id, first_name, last_name, email, mobile, wallet_balance FROM users ";
...
const cheackemailexistupdate = (email, userID, callback) => {
  ...
  db.query(query, [email, userID], (err, result) => {
    ...
    if (result.length > 0) {
      ...
      return callback({ status: 40, message: ["Email already exists"] });
    }
    callback(null, false);
  });
};
