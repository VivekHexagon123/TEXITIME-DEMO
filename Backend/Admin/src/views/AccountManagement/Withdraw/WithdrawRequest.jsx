...
const BasicButton = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/users/getwaitingrequest')
  ...
  const HandleEditbtn = async (id) => {
    ...
    // console.log(id);
    try {
      const response = await axios.get(`http://localhost:8000/api/users/getbankdetailes/${id}`);
  ...
  const handleApprovebtn = async (id) => {
    try {
      const response = await axios.put(`http://localhost:8000/api/users/approvedstaus/${id}`);
      if (response.status === 200) {
    ...
    <React.Fragment>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Footer>
          ...
          <Button variant="dark" onClick={() => handleApprovebtn(listusers[0].request_id)}>
            Approve
          </Button>
        </Modal.Footer>
      </Modal>
