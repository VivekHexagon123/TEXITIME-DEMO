...
const BasicButton = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/users/getapprovedrequest')
  ...
  const HandleEditbtn = async (id) => {
    ...
    // console.log(id);
    try {
      const response = await axios.get(`http://localhost:8000/api/users/getapprovedbankdetailes/${id}`);
  ...
  const handledisApprovebtn = async (id) =>{
    try {
         const response = await axios.put(`http://localhost:8000/api/users/disapprovedstaus/${id}`)
         if(response.status === 200) {
              ...
              <>
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div className="d-flex">
                    ...
                    <Button onClick={() => navigate('/account/withdrawdisapproved')} className="btn-dark">
                    DISAPPROVED 
                    </Button>
                  </div>
                </div>
