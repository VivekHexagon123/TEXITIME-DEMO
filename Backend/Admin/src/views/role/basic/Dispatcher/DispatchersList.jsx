...
const BasicButton = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/users//listdispatchers')
  ...
  const HandleEditbtn = async (userid) => {
    navigate(`/basic/DispatcherUpdate/${userid}`);
  };
  const handleDelete = async (id) => {
    console.log(id);

    try {
      const response = await fetch(`http://localhost:8000/api/users/deletedispatcher/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
              ...
              <>
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div className="d-flex">
                    ...
                    <Button onClick={() => navigate('/basic/AddNewDispatcher')} className="btn-dark">
                      Add New User
                    </Button>
                  </div>
                </div>
