...
const BasicButton = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/users/listfleetsowner')
  ...
  const HandleEditbtn = async (userid) => {
    navigate(`/basic/FleetownersUpdate/${userid}`);
  };
  const handleDelete = async (id) => {
    console.log(id);

    try {
      const response = await fetch(`http://localhost:8000/api/users/deletefleetowner/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
